...
class ContextBreak(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bbox = np.array([
            (x_center-0.5*w) / image_width  * self.search_size[1],
            (y_center-0.5*h) / image_height * self.search_size[0],
            w                / image_width  * self.search_size[1],
            h                / image_height * self.search_size[0]
        ]).astype(np.int32)
