...
#         print(self.annotation_counts.items())
#         print(self.id2idx)
...
class COCODatasetRandomRecord(COCODataset):
    """
    Provides the same functionality as COCODataset but random target crop within the same class is returned.
    This can be useful for in-depth attention map generated.
    """
    ...
    def __getitem__(self, idx):
        ...
        # 1. context file name
        context_file_name = self.id2file[self.annotations[idx]['image_id']]
        if context_file_name.split('/')[-1][:len(label_name)] == label_name:
            dict_key = '_'.join([label_name, context_file_name.split('/')[-1][len(label_name):]])
        else:
            dict_key = '_'.join([label_name, context_file_name.split('/')[-1]])
        
        # 2. target file name
        target_file_name = self.id2file[target_random_annotation['image_id']]
        if target_file_name.split('/')[-1][:len(label_name)] == label_name:
            dict_value = '_'.join([label_name, target_file_name.split('/')[-1][len(label_name):]])
        else:
            dict_value = '_'.join([label_name, target_file_name.split('/')[-1]])
        
        # normalize
        if self.normalize:
            target_image = normalize(target_image, self.normalize_means, self.normalize_stds)        
        
        return image, target_image, bbox_relative, label, (dict_key, dict_value), (idx, self.category_idx_dict[label_name][random_idx])
...
class COCODatasetMixOR(COCODataset):
    """
    Provides the same functionality as COCODataset but mix target crop(random or original) within the same class is returned.
    This can be useful for in-depth attention map generated.
    """

    def __getitem__(self, idx):
        # choice for context and target target
        # context choice: 0 -> random, 1 -> original 
        # target choice: 0 -> random, 1 -> original
        choice_context, choice_target, target_image = random.choice([0, 1]), random.choice([0, 1]), None
...
class COCODatasetCustom(COCODataset):
    """
    Provides the same functionality as COCODataset but mix target crop(random or empty or original) within the same class is returned.
    This can be useful for in-depth attention map generated.
    """


    def __init__(self, annotations_file, image_dir, image_size, category_dic_dir=None, idx2label=None, normalize_means=None, normalize_stds=None):
        super().__init__(annotations_file, image_dir, image_size, category_dic_dir, idx2label, normalize_means, normalize_stds)

        self.context_choice = 0
    ...
    def __getitem__(self, idx):
        ...
        choice_context = self.context_choice
        choice_target  = self.target_choice
        ...
        if choice_context == 1:
            image[:, ymin_og:ymin_og+h_og, xmin_og:xmin_og+w_og] = 0
        # _____ CONTEXT PROCESSING PART _____s
            
        return image, target_image, bbox_relative, label
    
    
class COCODatasetFullMix(COCODataset):
    """
    Provides the same functionality as COCODataset but mix target crop(random or empty or original) within the same class is returned.
    This can be useful for in-depth attention map generated.
    """
...
class COCODatasetZeroContextOriginalTarget(COCODataset):
    """
    Provides the same functionality as COCODataset but zero target crop and context with zero in target area within the same class is returned.
    This can be useful for in-depth attention map generated.
    """
    ...
    def __getitem__(self, idx):    
        # info inherited from the COCODataset
        image, target_image, bbox_relative, label = super().__getitem__(idx)
        label_name = self.idx2label[label]
