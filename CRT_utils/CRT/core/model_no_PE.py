...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.target_stream = torch.cuda.Stream(priority=-1) if self.gpu_streams else None
    ...
    def from_config(cls, cfg, num_classes=None, extended_output=False, gpu_streams=True):
        ...
        return cls(
            ...
            uncertainty_gate_type = cfg.uncertainty_gate_type,
            uncertainty_threshold = cfg.uncertainty_threshold, 
            weighted_prediction   = cfg.weighted_prediction, 
...
#     def forward_test(self, context_images, target_encoding_cate, target_bbox):
    ...
    def forward_test(self, context_images, target_encoding_cate):
...
#             context_encoding, target_encoding, target_bbox)
        ...
        context_encoding, target_encoding = self.positional_encoding(context_encoding, target_encoding)
...
#                          ).permute(1, 0, 2)
    ...
    def bbox2token(self, bbox):
        ...
        """
        token_ids = (
            (torch.ceil((bbox[:, 0] + bbox[:, 2] / 2) * self.tokens_per_dim) - 1) 
          + (torch.ceil((bbox[:, 1] + bbox[:, 3] / 2) * self.tokens_per_dim) - 1) * self.tokens_per_dim
        ).long()
