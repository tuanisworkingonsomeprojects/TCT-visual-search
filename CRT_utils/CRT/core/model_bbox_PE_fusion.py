...
class Model(nn.Module):
    ...
    def __init__(self, num_classes, num_decoder_layers=6, num_decoder_heads=8, uncertainty_gate_type="entropy", uncertainty_threshold=0, weighted_prediction=False, extended_output=False, gpu_streams=True):
        ...
        self.context_gate = nn.Sequential(
            nn.Linear(self.context_encoder.NUM_FEATURES, self.context_encoder.NUM_FEATURES),
            nn.Sigmoid()
        )
        # _____ MODIFIED CODE _____
    ...
    def forward(self, context_images, target_images, target_bbox):
        ...
        # ==============================
        target_tokens = target_encoding.permute(1, 0, 2)     # (B, T_tgt, D)
        context_tokens = context_encoding.permute(1, 0, 2)   # (B, T_ctx, D)

        # ==============================
        # Use LAST decoder layer attention
        # ==============================
        last_layer_attn = attention_map[:, -1]   # (B, T_tgt, T_ctx)

        # ==============================
        # Compute attended context tokens
        # ==============================
        attended_context_tokens = torch.bmm(
            last_layer_attn,
            context_tokens
        )   # (B, T_tgt, D) <- (T_tgt, T_ctx) x (T_ctx, D) batch-wise multiplication

        # ==============================
        # Pool → single context vector
        # ==============================
        attended_context = attended_context_tokens.mean(dim=1)   # (B, D)

        # ==============================
        # Pool target tokens
        # ==============================
        target_feat = target_tokens.mean(dim=1)   # (B, D)

        # ==============================
        # Context gating
        # ==============================
        gate = self.context_gate(attended_context)   # (B, D)

        fused = target_feat * gate                   # (B, D)

        # ==============================
        # Classification
        # ==============================
        main_prediction = self.classifier(fused)     # (B, C)
...
#         self.encoder = torchvision.models.densenet169(pretrained=True).features
        ...
        self.encoder = torchvision.models.densenet169(weights=DenseNet169_Weights.IMAGENET1K_V1).features
