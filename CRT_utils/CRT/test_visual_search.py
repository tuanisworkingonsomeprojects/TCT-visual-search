...
from .core.dataset import COCODatasetWithID, COCODataset
from .core.config import save_config
from .core.model import Model
# from .core.metrics import AccuracyLogger, IndividualScoreLogger

def bbox2token(model, bbox):
    """
    ...
    tokens_per_dim = int(math.sqrt(model.NUM_CONTEXT_TOKENS))
    
    token_ids = (
        (torch.ceil((bbox[:, 0] + bbox[:, 2] / 2) * tokens_per_dim) - 1) 
      + (torch.ceil((bbox[:, 1] + bbox[:, 3] / 2) * tokens_per_dim) - 1) * tokens_per_dim
    ).long()
...
#         individual_scores = IndividualScoreLogger(testset.idx2label)
    ...
    with torch.no_grad():
        for i, (context_images, target_images, bbox, labels_cpu) in enumerate(tqdm(dataloader, desc="Test Batches", leave=True)):
...
#             output = model(context_images, target_images) # output is (batchsize, num_classes) tensor of logits
            ...
            attention_map = attention_map[:, model.NUM_DECODER_LAYERS - 1, 0]
            # _____ MODIFIED VERSION _____
            
            predictions = torch.argmax(attention_map.detach().to('cpu'), 1)
        
#             _, predictions = torch.max(output.detach().to("cpu"), 1) # choose idx with maximum score as prediction
#             test_accuracy.update(predictions, bbox2token(model, bbox))
...
#             test_accuracy.update(predictions, labels_cpu)
            ...
            # print
            if print_batch_metrics:
                batch_loss = criterion(attention_map, bbox2token(model, bbox)).item()
                batch_corr = sum(predictions == bbox2token(model, bbox)) # number of correct predictions
