...
from .core.dataset import COCODatasetWithID, COCODataset, COCODatasetZeroContextZeroTarget
...
#             output = model(context_images, target_images, bbox) # output is (batchsize, num_classes) tensor of logits
            ...
            # _____ MODIFIED VERSION _____
            output, attention_map = model(context_images, target_images, bbox) # output is (batchsize, num_classes) tensor of logits
            # _____ MODIFIED VERSION _____
...
#             _, predictions = torch.max(output.detach().to("cpu"), 1) # choose idx with maximum score as prediction
            ...
            predictions = torch.argmax(output.detach().to('cpu'), 1)
